...
inline void addBook(QSqlQuery &q, const QString &title, int year, const QVariant &authorId,
...
inline QVariant addGenre(QSqlQuery &q, const QString &name)
{
    q.addBindValue(name);
    q.exec();
...
inline QVariant addAuthor(QSqlQuery &q, const QString &name, QDate birthdate)
{
    q.addBindValue(name);
...
inline QSqlError initDb(const QString &dbPath = QStringLiteral("/SD:/books.db"))
{
    QSqlDatabase db = QSqlDatabase::addDatabase("QSQLITE");
    db.setDatabaseName(dbPath);
    printk("[books] opening DB: %s\n", qPrintable(dbPath));
    ...
    if (tables.contains("books", Qt::CaseInsensitive)
        ...
        printk("[books] existing DB found, skipping init\n");
        return QSqlError();
    }
    printk("[books] creating tables + inserting initial data\n");
